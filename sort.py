...
from utils.general import (
    ...
    xyxy2xywh, plot_one_box_with_dot, strip_optimizer, set_logging)
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if wasteType == 0:
                            if midX in plasticXRange:
                                if abs(datetime.datetime.now().time().second - PlasticLastCall) > 2:
                                    PlasticLastCall = datetime.datetime.now().time().second
                                    send('P')
                                    print("Plastic piston Fired")
                        elif wasteType == 1:
                            if midX in aluminiumXRange:
                                if abs(datetime.datetime.now().time().second - AluminiumLastCall) > 2:
                                    AluminiumLastCall = datetime.datetime.now().time().second
                                    send('A')
                                    print("Aluminum piston Fired")
                        elif wasteType == 2:
                            if midX in paperXRange:
                                if abs(datetime.datetime.now().time().second - PaperLastCall) > 2:
                                    PaperLastCall = datetime.datetime.now().time().second
                                    send('G')
                                    print("Paper piston Fired")

                        # Plot a box with midpoint
                        plot_one_box_with_dot(xyxy, im0, (midX, midY), label=str(wasteType), color=colors[int(cls)], line_thickness=3)
